...
export default class BookDonateScreen extends React.Component{
    ...
    render(){
        return(
            <View style={{flex:1}}>
            <MyHeader title={firebase.auth().currentUser.email+ "'s  Donate Books:"}></MyHeader>
            <View style={{flex:1}}>
            {
              this.state.requestedBookList.length===0
              ?
              (
              <Text style={{flex:1, fontSize: 20, justifyContent: 'center', alignItems: 'center'}}>{"List of Books Requested By Our Users  would be here, " + firebase.auth().currentUser.email + ". However, as of now, there are no pending requests. To make one, go to the exchange screen."}</Text>
              )  
              :
              (
